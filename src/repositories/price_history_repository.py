...
class PriceHistoryRepository:
    ...
    async def save(self, entity: dict):
        try:
            ...
            new_entity = PriceHistoryEntity(**entity)
    ...
    async def get_by_id(self, entity_id: str):
        ...
        entity = await PriceHistoryEntity.get(ObjectId(entity_id))
        if not entity:
    ...
    async def delete(self, test_id: str):
        try:
            ...
            test = await PriceHistoryEntity.get(PydanticObjectId(test_id))
            if not test:
    ...
    async def update(self, test_id: str, update_data: dict):
        try:
            ...
            result = await PriceHistoryEntity.find_one(
                PriceHistoryEntity.id == PydanticObjectId(test_id)
